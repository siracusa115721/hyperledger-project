export CHANNEL_NAME=channel-equipos
export CHAINCODE_NAME=equipos_chaincode
...
# Commit en los peers
# RealMadrid
peer lifecycle chaincode approveformyorg --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --waitForEvent --signature-policy "OR ('RealMadridOrgMSP.peer','BarcelonaOrgMSP.peer')" --package-id $code_id
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --signature-policy "OR ('RealMadridOrgMSP.peer','BarcelonaOrgMSP.peer')" --output json

# Barcelona
CORE_PEER_MSPCONFIGPATH=${ROOT}/crypto/peerOrganizations/barcelona.org.laliga.com/users/Admin@barcelona.org.laliga.com/msp CORE_PEER_ADDRESS=peer0.barcelona.org.laliga.com:7051 CORE_PEER_LOCALMSPID="BarcelonaOrgMSP" CORE_PEER_TLS_ROOTCERT_FILE=${ROOT}/crypto/peerOrganizations/barcelona.org.laliga.com/peers/peer0.barcelona.org.laliga.com/tls/ca.crt peer lifecycle chaincode approveformyorg --tls --cafile ${ROOT}/crypto/ordererOrganizations/laliga.com/orderers/orderer.laliga.com/msp/tlscacerts/tlsca.laliga.com-cert.pem --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --waitForEvent --signature-policy "OR ('RealMadridOrgMSP.peer','BarcelonaOrgMSP.peer')" --package-id $code_id
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --signature-policy "OR ('RealMadridOrgMSP.peer','BarcelonaOrgMSP.peer')" --output json

# commit
peer lifecycle chaincode commit -o orderer.laliga.com:7050 --tls --cafile $ORDERER_CA --peerAddresses peer0.realmadrid.org.laliga.com:7051 --tlsRootCertFiles ${ROOT}/crypto/peerOrganizations/realmadrid.org.laliga.com/peers/peer0.realmadrid.org.laliga.com/tls/ca.crt --peerAddresses peer0.barcelona.org.laliga.com:7051 --tlsRootCertFiles ${ROOT}/crypto/peerOrganizations/barcelona.org.laliga.com/peers/peer0.barcelona.org.laliga.com/tls/ca.crt --channelID $CHANNEL_NAME --name $CHAINCODE_NAME --version $CHAINCODE_VERSION --sequence 1 --signature-policy "OR ('RealMadridOrgMSP.peer','BarcelonaOrgMSP.peer')"
